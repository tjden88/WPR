...
/// </summary>
internal class BuilderContext(ActionHelper? actionHelper)
{
    /// <summary> Нужен, если цепочка началась из инстанса ActionHelper, для корректного выполнения </summary>
    public ActionHelper? ActionHelper { get; } = actionHelper;
...
public class ActionHelper
{
    ...
    /// </summary>
    public static ActionBuilder StartNew() => new(new BuilderContext(null));

    /// <summary>
    /// Начать построение новой цепочки.
    /// </summary>
    public ActionBuilder Start() => new(new BuilderContext(this));

    /// <summary>
    /// Начать построение новой цепочки с начальной проверки.
    /// </summary>
    public CheckActionBuilder Check(Func<bool> condition) => Start().Check(condition);

    /// <summary>
    /// Начать построение новой цепочки с начальной асинхронной проверки.
    /// </summary>
    public CheckActionBuilder Check(Func<CancellationToken, Task<bool>> condition) => Start().Check(condition);
