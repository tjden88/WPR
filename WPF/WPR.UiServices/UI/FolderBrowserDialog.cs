...
public class FolderBrowserDialog
{
    ...
    public bool ShowDialog(IWin32Window owner)
    {
        ...
        frm.SetTitle(Title ?? "Выбор папки");
        ...
        if (frm.Show(owner.Handle) != NativeMethods.S_OK) return false;
        if (frm.GetResult(out var shellItem) != NativeMethods.S_OK) return false;
        if (shellItem.GetDisplayName(NativeMethods.SIGDN_FILESYSPATH, out var pszString) != NativeMethods.S_OK) return false;
        if (pszString == IntPtr.Zero) return false;
